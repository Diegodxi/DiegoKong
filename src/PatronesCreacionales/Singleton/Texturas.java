...
import SistemaGFX.CargadorImagenes;
...
public final class Texturas {
    ...
    private BufferedImage jugador_sheet, barril_sheet;
    ...
    private BufferedImage[] mario_s, mario_martillo;
    ...
    private void cargarSpritesGlobales() {
        ...
        jugador_sheet = cargar.loadImage(folder + "/testt.png");
        ...
        getBarrilTexturas();
        getItemsTexturas();
        getJugadorMartilloTexturas();
        getDiegoKongTexturas();
        getPrincesaTexturas();
        getFuegoTexturas();
        getLlamaTexturas();
        getJugadorMuerteTexturas();
    }
    ...
    private void getJugadorTexturas() {
        int x_off = 1, y_off = 1, width = 16, height = 16;
        for (int i = 0; i < mario_S_count; i++) {
            mario_s[i] = jugador_sheet.getSubimage(x_off + i * (width + 2), y_off, width, height);
        }
    }
    
    private void getJugadorMartilloTexturas() {
        int x_off = 1, y_off = 73, width = 32, height = 32;
        try {
            for (int i = 0; i < mario_martillo_count; i++) {
                mario_martillo[i] = jugador_sheet.getSubimage(
    ...
    private void getJugadorMuerteTexturas() {
        int x_off = 1, y_off = 37, width = 16, height = 16;
        for (int i = 0; i < mario_muerte_count; i++) {
            mario_muerte[i] = jugador_sheet.getSubimage(x_off + i * (width + 2), y_off, width, height);
        }
    }
